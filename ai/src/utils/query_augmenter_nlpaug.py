...
class QueryAugmenterNlpAug:
    PATTERNS = {
        ...
        'FUNC': re.compile(r'[a-zA-Z0-9]+ function', re.IGNORECASE)
    }
    ...
    def augment(self, 
        ...
        for input_text, output_text in tqdm(zip(input_texts, output_texts)):
            # Store original
            unique_pairs[input_text] = output_text
                ...
                try:
                    for _ in range(num_augments_per_method):
                        ...
                        if isinstance(augmented, str) and '[unk]' not in augmented.lower():
                            if augmented != input_text:  # 중복 제거
                                unique_pairs[augmented] = output_text
                                augmented_count += 1
                except Exception as e:
        ...
        logging.info(f"Total unique samples: {len(unique_pairs)}")
